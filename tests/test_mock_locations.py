...
def test_get_all_locations(mocker, client: HomeboxClient):
    mocker.patch.object(
        client, "_request", return_value=[{"id": "1", "name": "Test Location"}]
    )
...
def test_create_location(mocker, client: HomeboxClient):
    ...
    result = client.locations.create_location(
        models.LocationCreate(name="Test Location")
    )
...
def test_update_location(mocker, client: HomeboxClient):
    mocker.patch.object(
        client, "_request", return_value={"id": "1", "name": "Updated Location"}
    )
    result = client.locations.update_location(
        "1", models.LocationUpdate(name="Updated Location")
    )
